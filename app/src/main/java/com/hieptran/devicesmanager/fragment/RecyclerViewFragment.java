 ...
 */
public class RecyclerViewFragment extends CommonFragment {
    ...
    private CustomAdapter.Adapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, final Bundle savedInstanceState) {
        ...
        Log.i(Const.TAG, "Opening " + getClassName());
...
*/
       /* if (fabView != null && Utils.isTV(getActivity())) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPreExecute() {
                ...
                adapter = new CustomAdapter.Adapter(new ArrayList<CustomAdapter.AdapView>());
                try {
    ...
    public void addView(CustomAdapter.AdapView view) {
        if (adapter.AdapViews.indexOf(view) < 0) {
            adapter.AdapViews.add(view);
            adapter.notifyDataSetChanged();
        }
    }

    public void removeView(CustomAdapter.AdapView view) {
        int position = adapter.AdapViews.indexOf(view);
        if (position > -1) {
            adapter.AdapViews.remove(position);
            adapter.notifyItemRemoved(position);
        }
    }

    public void removeAllViews() {
        adapter.AdapViews.clear();
        adapter.notifyDataSetChanged();
    }

    public void addAllViews(List<CustomAdapter.AdapView> views) {
        adapter.AdapViews.addAll(views);
        adapter.notifyDataSetChanged();
    }

    public CustomAdapter.AdapView getView(int position) {
        return adapter.AdapViews.get(position);
    }
